...
See [wikipedia](https://en.wikipedia.org/wiki/Dallas%E2%80%93Fort_Worth_metroplex) 
for a list of counties within the Dallas–Fort Worth–Arlington MSA.
```{r message=FALSE}
...
#  -> Make call
# 
# (1) request_ed_by_sex_race_tract
# (2) request_ed_by_age_tract


# Functions to create dataframes from API returns
# -----------------------------------------------
#  -> Call function to make API call
#  -> Aggregate estimates as needed
#
# (1) create_ed_by_sex_race_tract_df
# (2) create_ed_by_age_tract_df


# Master function to make all API calls and create dataframes
# -----------------------------------------------------------
#  -> If dataset does not exist on disk:
...
#       -> Load from disk
#
# (1) make_all_education_api_calls
```
...
# ----------------------------------------------------------------
request_ed_by_sex_race_tract <- function(raceth, county, year){
...
# ----------------------------------
request_ed_by_age_tract <- function(county, year){
...
# DEPRECATED -- not used in calculating joint probability

# request_age_by_tract <- function(raceth, county){
...
request_below_poverty_by_tract <- function(raceth, county, year){
...
# ----------------------------------------------------------------
create_ed_by_sex_race_tract_df <- function(county, year){
  # Educational Attainment
  white <- request_ed_by_sex_race_tract('white', county, year) %>% mutate(race = 'white')
  black <- request_ed_by_sex_race_tract('black', county, year) %>% mutate(race = 'black')
  alaskan <- request_ed_by_sex_race_tract('alaskan', county, year) %>% mutate(race = 'alaskan')
  asian <- request_ed_by_sex_race_tract('asian', county, year) %>% mutate(race = 'asian')
  hawaian <- request_ed_by_sex_race_tract('hawaian', county, year) %>% mutate(race = 'hawaian')
  other <- request_ed_by_sex_race_tract('other', county, year) %>% mutate(race = 'other')
  multirace <- request_ed_by_sex_race_tract('multirace', county, year) %>% mutate(race = 'multirace')
  hispanic <- request_ed_by_sex_race_tract('hispanic', county, year) %>% mutate(race = 'hispanic')
  ...
  ed <- ed %>% 
    ...
    distinct(year, county, tract, education, sex, race, .keep_all=TRUE)
...
# 
# ------------------------------------------------------------------
create_ed_by_age_tract_df <- function(county, year){
  age <- request_ed_by_age_tract(county, year)
  ...
  age <- age %>% 
    ...
    group_by(tract, education, agecat) %>% 
    mutate(estimate = sum(estimate),
           moe = abs(sqrt( sum(moe^2) ))
    ) %>% 
    ungroup(tract, education, agecat) %>%
    select(year, county, tract,
           education, agecat, estimate, moe) %>% 
    distinct() %>% 
    group_by(tract, agecat) %>% 
    mutate(denom = sum(estimate),
           moe_denom = abs(sqrt( sum(moe^2) ))
           ) %>% 
    ungroup(tract, agecat) %>% 
    mutate(proportion = estimate/denom,
           proportion_moe = abs(sqrt((moe^2) - ((estimate/denom)^2 * moe_denom)))/denom
    ) %>%
    distinct(year, county, tract, education, agecat, .keep_all=TRUE)
...
# -----------------------------------------------------------------------------
create_poverty_by_tract_df <- function(county, year){
  # Educational Attainment
  white <- request_below_poverty_by_tract('white', county, year) %>% mutate(race = 'white')
  black <- request_below_poverty_by_tract('black', county, year) %>% mutate(race = 'black')
  alaskan <- request_below_poverty_by_tract('alaskan', county, year) %>% mutate(race = 'alaskan')
  asian <- request_below_poverty_by_tract('asian', county, year) %>% mutate(race = 'asian')
  hawaian <- request_below_poverty_by_tract('hawaian', county, year) %>% mutate(race = 'hawaian')
  other <- request_below_poverty_by_tract('other', county, year) %>% mutate(race = 'other')
  multirace <- request_below_poverty_by_tract('multirace', county, year) %>% mutate(race = 'multirace')
  hispanic <- request_below_poverty_by_tract('hispanic', county, year) %>% mutate(race = 'hispanic')
...
##################################################################
#                          Load data 
#               or Make API calls and write to disk
##################################################################
...
#pov_filepath <- file.path('data', 'poverty_status_12m.csv')



make_all_education_api_calls <- function() {
  # For each YEAR IN (2010, 2020) and COUNTY in the DFW metroplex (two MSAs),
  #   make an API call requesting educational attainment estimates:
  #     -> by (SEX, RACE, CENSUS TRACT)
  #     -> by (AGE, CENSUS TRACT)
  #
  # Census tracts are updated every year ... however, patient addresses in EHR 
  #   are geocoded to the Decennial Census:
  #     -> 2020 data is required for patients whose address was last
  #         updated/verified after January 2020
  #
  #     -> 2010 data is required for all other patients
  
  # ACS data years
  years = c(2010, 2020)
  
  dfw_counties <- c(
    # MSA : Fort Worth–Arlington–Grapevine metropolitan division
    'Johnson County', 'Parker County', 'Tarrant County', 'Wise County', 
    # MSA : Dallas–Plano–Irving metropolitan division
    'Collin County', 'Dallas County', 'Denton County', 'Ellis County', 
    'Hunt County', 'Kaufman County', 'Rockwall County')
  
  # Make calls for education by (SEX, RACE, TRACT)
  # ----------------------------------------------
  ed <- data.frame()
  ...
  for(year in years){
    for(county in dfw_counties){
      ...
      # make api call
      ed.year.county <-
        create_ed_by_sex_race_tract_df(county, year)

      # combine
      ed <- rbind(ed, ed.year.county)

    }
  }

  # Make calls for education by (AGE, TRACT)
  # ----------------------------------------
  age <- data.frame()
  ...
  for(year in years){
    for(county in dfw_counties){
      ...
      # make api call
      age.year.county <- 
        create_ed_by_age_tract_df(county, year)
      
      # combine (bind_rows)
      age <- rbind(age, age.year.county)
  ...
  # Write to disk
  # --------------
  readr::write_excel_csv(ed, ed_filepath, delim = '\t')
  readr::write_excel_csv(age, age_filepath, delim = '\t')
...
# --------------------------------
if(file.exists(ed_filepath) & file.exists(age_filepath)){
  ...
  df <- make_all_education_api_calls()
}
...
# } else {
#   # MSA : Fort Worth–Arlington–Grapevine metropolitan division
#   pov.johnson <- create_poverty_by_tract_df('Johnson County')
#   pov.parker <- create_poverty_by_tract_df('Parker County')
#   pov.tarrant <- create_poverty_by_tract_df('Tarrant County')
#   pov.wise <- create_poverty_by_tract_df('Wise County')
# 
#   # MSA : Dallas–Plano–Irving metropolitan division
#   pov.collin <- create_poverty_by_tract_df('Collin County')
#   pov.dallas <- create_poverty_by_tract_df('Dallas County')
#   pov.denton <- create_poverty_by_tract_df('Denton County')
#   pov.ellis <- create_poverty_by_tract_df('Ellis County')
#   pov.hunt <- create_poverty_by_tract_df('Hunt County')
#   pov.kaufman <- create_poverty_by_tract_df('Kaufman County')
#   pov.rockwall <- create_poverty_by_tract_df('Rockwall County')
# 
#   pov <- rbind(pov.johnson, pov.parker, pov.tarrant, pov.wise,
#               pov.collin, pov.dallas, pov.denton, pov.ellis,
#               pov.hunt, pov.kaufman, pov.rockwall)
#   
#   rm(list = c('pov.johnson', 'pov.parker', 'pov.tarrant', 'pov.wise',
#               'pov.collin', 'pov.dallas', 'pov.denton', 'pov.ellis',
#               'pov.hunt', 'pov.kaufman', 'pov.rockwall'))
#   # write out
...
ggplot(dat,
  ...
  labs(
    ...
    y = 'Number of persons',
    title = 'Male -- Tract 1115.67 (year: 2020, county: Tarrant)',
    fill = 'Race/Ethnicity') + 
...
estimates <- ed %>% 
  filter(education != 'Total') %>% 
  select(year, county, tract, education, sex, race, estimate, moe, proportion, proportion_moe) %>% 
  left_join(age %>% select(year, county, tract, education, agecat, proportion, proportion_moe) %>% 
...
ggplot(dat,
  ...
  labs(
    x = 'Highest Level of Education Achieved',
    y = 'Proportion',
    title = 'Female -- Tract 1115.67 (year: 2020, county: Tarrant)',
    fill = 'Age Category') + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=0.5),
        plot.title = element_text(hjust = 0.5))
```
...
##### Males
Updated API calls have not yet been made (by year or with additional counties).
