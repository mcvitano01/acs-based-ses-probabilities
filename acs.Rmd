...
The user will need to signup for their own US Census [API key](https://api.census.gov/data/key_signup.html).
...
##################################################################
if(!require(rgdal)) install.packages('tidycensus')
if(!require(leaflet)) install.packages('dplyr')
if(!require(vroom)) install.packages('stringr')
...
# user will need to create their own US Census API key
